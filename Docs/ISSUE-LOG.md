...
| #1 | not yet filed | `Tests/foobarTests/TestHelpers.swift:47` warns that the result of `configureServer` is unused. Harmless today; it would block ever enabling `--warnings-as-errors`. **Still live as of 2026-08-16** — #7 closed without touching it, and it is not formatting, so it does not belong to #6 either. SwiftPM only re-emits it when that file recompiles, which is why it is easy to believe it has gone away. |
