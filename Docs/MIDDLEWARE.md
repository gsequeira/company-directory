...
is planned. Structured so you can add further middleware as they become worth having.

**Written 2026-08-16**, because two open issues both resolve to "write a middleware": #24
(authentication) and #2 (malformed input returns `500` instead of `400`). Neither had anywhere to
explain why that is the right layer.
...
A middleware sits between the server and the router, and can read or alter the request on the way in,
the response on the way out, or answer immediately without the router ever running.
...
short-circuits the request. Everything after that call runs on the way *out*, which is where you
would alter a response.
...
So the future-based protocol is what the server still speaks, and conforming to `AsyncMiddleware` is
what keeps it out of your code. Worth knowing only because it explains why two protocols appear in
the documentation for one concept, and because a compiler error mentioning `EventLoopFuture<Response>`
usually means someone wrote the conformance against `Middleware` by mistake.
...
handlers means ten chances to forget it, and it would not run at all for a request that never reaches
a handler.

That is the shape of both open issues: rejecting an unauthenticated request before it touches the
database, and turning a decoding failure into the right status when there is no handler to return
one.
...
The second is why a thrown error becomes a `500` rather than a dropped connection. As *Planned, error
mapping* below explains, it is also why that `500` is not a `400`.
...
**That distinction is already load-bearing.** `routes(application)` registers `/health` directly on
the application, outside any group. Protecting the API by grouping the transport leaves health probes
reachable without a single line of exemption logic. The separation exists for unrelated reasons and
...
The array is wrapped from the back, so **the first middleware registered is the outermost**, first to
see the request and last to see the response. Anything that must observe errors thrown by another
...
`constantTimeEquals` in the example above is ours to write. swift-crypto has `constantTimeCompare`,
but it is `internal`, which I checked rather than assumed, so it cannot be called from here. The loop
is short:
...
of the layer rather than a bug. Worth stating in the spec rather than leaving someone to discover it.

**Note also that `swift-openapi-generator` supports neither `security` nor `securitySchemes`**, both
unchecked in its feature matrix. Declaring them generates and enforces nothing. The declaration is
documentation for humans and other tooling; the middleware is what enforces.
...
error, a 922-byte response, observed.

The fix is a middleware registered **outside** the error middleware, recognising the decoding failure
and rethrowing it as `Abort(.badRequest)`. You cannot make the error type conform to `AbortError`
from here, because it belongs to a dependency.

#14 holds the acceptance criteria. Those tests exist now inside `withKnownIssue`, assert the `400`
the server should return, and fail the moment this middleware makes them pass. See
[`TESTING.md`](TESTING.md) → *Step 7*.

## Planned, tracing (#45)

Vapor's `TracingMiddleware` opens a span per request. Registering it is one line, but the interesting
part is how little else is missing.

### Most of the instrumentation is already here, and inert

I checked these against the dependency sources rather than assuming:
...
[`FLUENT.md`](FLUENT.md) says to detect N+1 by counting queries per request rather than by how fast it
feels at these row counts. With a backend bootstrapped, **an N+1 becomes something you can look at**:
one server span containing N `fluent.query` client spans, where there should be two.

That is a better lesson than the paragraph describing it, and Phase 2 walks straight into the problem,
so the timing matters. #18 as scoped returns `departmentId`, a column already on the row, so no N+1
exists yet. The nested-collection route in [`API-DESIGN.md`](API-DESIGN.md) §2.6, or a richer
`Employee` response, is what introduces one.

### What it costs

- **A collector to export to**, which is one `docker-compose` service.
...
| --- | --- | --- |
| `CORSMiddleware` | Yes | A browser client exists. Not before, since it is a header policy for browsers and does nothing for server-to-server callers |
...
Do **not** reach for a middleware when the work needs to know *which* operation it is handling. A
middleware sees a raw `Request`, meaning method, path, headers and bytes, rather than the decoded,
...
every request the suite sends exercises a middleware registered in `configureServer`.
